...
namespace Microsoft.Extentions.Configuration.ConfigFile
{
    ...
    public class KeyValueParser : IConfigurationParser
    {
        ...
        public KeyValueParser(string key, string value) : this(key, value, null)
        { }
        ...
        public void ParseElement(XElement element, Stack<string> context, SortedDictionary<string, string> results)
        {
            ...
            {
                var hasSupportAction = node.DescendantsAndSelf().Any(x => _supportedActions.Contains(x.Name.ToString().ToLowerInvariant()));
                if (!hasSupportAction)
                {
                    if (_logger != null)
                    {
